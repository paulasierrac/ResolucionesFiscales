...
def obtener_config() -> dict:
    ...
    El ambiente de BD (Dev/Prod) se elige segun RPA_DEBUG en .env: True -> Dev-*,
    False -> Prod-* (secretos compartidos entre proyectos, tag environment=dev/prod).

    Requiere en .env: VAULT_URL, TENANT_ID, CLIENT_ID, CLIENT_SECRET, DB_SCHEMA, RPA_DEBUG.
    """
    import sys as _sys
    ...
    secretos = CargarVault(
        nombres=[f"{prefijo}-DBName", f"{prefijo}-DBPassword", f"{prefijo}-DBServer", f"{prefijo}-DBUser"],
        strip_prefix=prefijo,
    )

    schema = os.environ.get("DB_SCHEMA", "ResolucionesFiscales")
...
def enviar_correo(
    ...
    columnas Num_Correo, HU, Actividad, Caso, Para, Asunto, Contenido, ArchivoAdjunto).

    Lee la fila de [Correos] con Num_Correo = i_num_correo, reemplaza placeholders
    en asunto/cuerpo con i_replace_in_*, opcionalmente agrega el pie compartido
    (Num_Correo='0') y envia el correo por SMTP.

    Si no existe fila para i_num_correo (o no tiene destinatario), cae a
    in_config{MailTo} + i_asunto_fallback/i_contenido_fallback.
    ...
    write_log("Info", f"Inicio envio correo Num_Correo={i_num_correo}", task_name, in_config)
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT Para, Asunto, Contenido FROM {esquema}.{tabla} WHERE Num_Correo = ?",
            (str(i_num_correo),)
        )
        row = cursor.fetchone()
        pie = ""
        if i_incluir_pie and str(i_num_correo) != "0":
            cursor.execute(
                f"SELECT Contenido FROM {esquema}.{tabla} WHERE Num_Correo = '0'"
            )
            fila_pie = cursor.fetchone()
        ...
        if row is not None and row[0]:
            ...
            write_log(
                "Warning",
                f"enviar_correo: sin registro/destinatario para Num_Correo={i_num_correo} "
                f"en {esquema}.{tabla}; usando MailTo + asunto/contenido de respaldo.",
                task_name, in_config,
            )
        ...
        if not to:
            msg_err = f"enviar_correo: no hay destinatario para Num_Correo={i_num_correo} y no hay MailTo configurado."
        ...
        destinatarios = [d.strip() for d in to.replace(";", ",").split(",") if d.strip()]
        ...
        write_log("Info",
                  f"Correo enviado correctamente Num_Correo={i_num_correo} -> {destinatarios}",
                  task_name, in_config)

    except Exception as e:
        out_system_exception = str(e)
        write_log("Error", f"enviar_correo Num_Correo={i_num_correo}: {e}", task_name, in_config)
